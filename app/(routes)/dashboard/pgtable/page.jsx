...
const PGTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const degreeRef = collection(db, "PG");
        ...
        for (const degreeDoc of degreeSnapshot.docs) {
          ...
          const coursesRef = collection(db, `PG/${universityId}/courses`);
          ...
          for (const { courseId, courseName } of courses) {
            const semestersRef = collection(
              db,
              `PG/${universityId}/courses/${courseId}/semesters`
            );
            ...
            for (const { semesterId, semesterName } of semesters) {
              const subjectsRef = collection(
                db,
                `PG/${universityId}/courses/${courseId}/semesters/${semesterId}/subjects`
              );
              ...
              for (const subjectDoc of subjectsSnapshot.docs) {
                ...
                const unitsRef = collection(
                  db,
                  `PG/${universityId}/courses/${courseId}/semesters/${semesterId}/subjects/${subjectId}/units`
                );
  ...
  return (
    <div>
      <h1 className="text-3xl text-center mt-10 underline">PG TABLE</h1>
